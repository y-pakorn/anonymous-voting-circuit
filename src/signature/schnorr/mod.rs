...
use ark_ff::{
    ...
    to_bytes, FromBytes, ToConstraintField, UniformRand,
};
...
impl<C: ProjectiveCurve> ToBytes for Parameters<C> {
    #[inline]
...
impl<C: ProjectiveCurve> FromBytes for Parameters<C> {
    fn read<R: Read>(mut reader: R) -> IoResult<Self> {
        let generator = C::Affine::read(&mut reader)?;
        let salt = Option::<[u8; 32]>::read(&mut reader)?;

        Ok(Self { generator, salt })
    }
}
